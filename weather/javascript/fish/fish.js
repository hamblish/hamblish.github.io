...
async function getWeatherHero() {
    let response = await fetch("https://api.openweathermap.org/data/2.5/weather?id=5585010&units=imperial&appid=b0d07380ce5ea2770c47bb85b41d4544");
...
async function getWeatherForecast (){
    let response = await fetch("https://api.openweathermap.org/data/2.5/forecast?id=5585010&units=imperial&appid=b0d07380ce5ea2770c47bb85b41d4544")
    let data = await response.json();
    let forecastFive = document.getElementById("forecast-widget")
    let city = document.createElement("div")
    city.setAttribute("id", "forecast-city")
    city.innerHTML='<p class="stronger">FISH HAVEN</p><p class="lighter">ID</p>'
...
async function populateCityInfo(webJSON){
    ...
    const events = townsJSON.towns[2].events
